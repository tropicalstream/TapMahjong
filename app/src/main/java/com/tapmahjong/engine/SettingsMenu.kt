...
class SettingsMenu(private val engine: GameEngine, private val store: SettingsStore) {
    ...
    val items: List<SettingsItem> = listOf(
        ...
        SettingsItem("Layout", { Layout.from(store.difficulty).label }, adjust = { d ->
            store.difficulty = (store.difficulty + d).coerceIn(0, 2); engine.menuDiff = store.difficulty
        }),
        SettingsItem("Timer", { if (store.timerOn) "On" else "Off" }, adjust = {
            store.timerOn = !store.timerOn
        }),
        SettingsItem("Hint", { "" }, activate = { engine.doubleTap(); engine.hint() }),
        SettingsItem("Shuffle", { "" }, activate = { engine.doubleTap(); engine.shuffle() }),
        SettingsItem("Show Free Dots", { if (store.showLegal) "On" else "Off" }, adjust = {
        ...
        SettingsItem("New Deal", { "" }, activate = { engine.doubleTap(); engine.newGame() }),
        SettingsItem("Undo Match", { "" }, activate = { engine.doubleTap(); engine.undo() }),
